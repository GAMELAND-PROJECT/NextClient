...
/** Run once. Re-running this function revokes the currently active code. */
function initializeAllclientAccess() {
  const properties = PropertiesService.getScriptProperties();
  properties.setProperty('ALLCLIENT_ENABLED', 'true');
  properties.deleteProperty('ALLCLIENT_ADMIN_TOKEN');
...
function doPost(event) {
  ...
  return currentCodePage_(message);
}
...
function currentCodePage_(message) {
  const properties = PropertiesService.getScriptProperties();
  const enabled = properties.getProperty('ALLCLIENT_ENABLED') === 'true';
  const activeCode = properties.getProperty('ALLCLIENT_ACTIVE_CODE') || '';
...
<form method="post" action="${serviceUrl}" target="_top"><input type="hidden" name="action" value="toggle"><button class="button ${buttonClass}" type="submit">${buttonText}</button></form></main></body></html>`);
}
